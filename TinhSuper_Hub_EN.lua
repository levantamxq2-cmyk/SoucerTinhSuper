...
task.spawn(function()
    local lastCore -- lưu core hiện tại để tránh spam
    ...
    while true do
        ...
        pcall(function()
            ...
            -- ===== TRẠNG THÁI CHỜ FACTORY (KHÔNG GIẬT) =====
            ...
            if core ~= lastCore then
                lastCore = core
                _tp(factoryPos) -- TP 1 lần duy nhất
                task.wait(0.3)
            end
...
task.spawn(function()
    ...
    local waitPos = CFrame.new(
        -5496.17432, 313.768921, -2841.53027,
        .924894512, 0, .380223751,
        0, 1, 0,
        -.380223751, 0, .924894512
    )
    ...
    while true do
        ...
        pcall(function()
            ...
            for _, mob in pairs(workspace.Enemies:GetChildren()) do
                ...
                and (mob.HumanoidRootPart.Position - Root.Position).Magnitude < 2000 then
                    target = mob
                    break
                end
            end

            -- ⏳ KHÔNG CÓ QUÁI → CHỜ (KHÔNG TP)
            if not target then
                lastTarget = nil
                task.wait(0.6)
                return
...
task.spawn(function()
	...
	while true do
		...
		pcall(function()
			...
			local target
			for _, mob in pairs(workspace.Enemies:GetChildren()) do
				if table.find(ectoMobs, mob.Name)
				and mob:FindFirstChild("Humanoid")
			...
			-- ⏳ không có quái → đứng yên chờ (KHÔNG TP, KHÔNG GIẬT)
			if not target then
				currentTarget = nil
				task.wait(0.6)
				return
			...
			-- ⚔️ đánh liên tục
			repeat
				if not _G["AutoEctoplasm"] then break end
				if not target.Parent then break end
				task.wait()
				L_1_[4]["Kill"](target, true)
			until target.Humanoid.Health <= 0
...
task.spawn(function()
	...
	while true do
		...
		pcall(function()
			...
			-- 🟡 CHƯA SPAWN → CHỜ (KHÔNG TELEPORT)
			if not boss then
				task.wait(1)
				return
			end

			-- 🟢 CHỈ TELEPORT 1 LẦN KHI ĐỔI TARGET
...
task.spawn(function()
	while true do
		-- OFF → ngủ hẳn, không tốn tài nguyên
		...
		pcall(function()
			...
			-- chưa có boss → CHỜ, KHÔNG TP
			if not RipIndraTarget then
				task.wait(2)
				RipIndraRunning = false
				return
			end
			...
			repeat
				...
				if RipIndraTarget
				and RipIndraTarget.Parent
				and RipIndraTarget:FindFirstChild("Humanoid")
				and RipIndraTarget.Humanoid.Health > 0 then

					-- TP 1 lần ổn định
					if Root and RipIndraTarget:FindFirstChild("HumanoidRootPart") then
						_tp(RipIndraTarget.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
					end
				...
				task.wait(0.1)
			until not RipIndraTarget
			or not RipIndraTarget.Parent
...
-- === FIX NHẸ CHO AUTO FARM BONE (GIỮ NGUYÊN LOGIC) ===
-- Thêm nếu chưa định nghĩa Pos trong block này:
local Pos = CFrame.new(0, 50, 0) -- đứng trên đầu mob

-- (Đặt/đè phần spawn loop "no quest" bằng đoạn sau)
spawn(function()
    while wait() do
        if _G.Bone and (not _G.QuestBone and World3) then
            pcall(function()
                if workspace.Enemies:FindFirstChild("Reborn Skeleton")
                or workspace.Enemies:FindFirstChild("Living Zombie")
                or workspace.Enemies:FindFirstChild("Demonic Soul")
                or workspace.Enemies:FindFirstChild("Posessed Mummy") then
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        ...
                        and v.Humanoid.Health > 0 then
                            ...
                            repeat
                                ...
                                -- disable collision / movement on mob so bring works
                                pcall(function()
                                ...
                                -- DÙNG đúng hàm TP của file: _tp
                                -- và offset Pos (đứng trên đầu mob)
                                pcall(function()
                    ...
                    -- KHÔNG CÓ MOB: tắt bring và bay về điểm spawn (giữ logic gốc)
                    BonesBring = false
                    _tp(CFrame.new(-9506.234375, 172.130615234375, 6117.0771484375))
                end
            end)
        else
            BonesBring = false
